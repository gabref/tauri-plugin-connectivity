...
/// does not provide a preferred internet connection profile.
fn is_missing_profile_error(error: &windows::core::Error) -> bool {
   error.code().is_ok()
}

/// Treat only explicit internet access as connected.
///
...
/// can still represent local-only or constrained access. The plugin's
/// `connected` field is intended to mean active internet reachability, so
/// non-internet levels map to disconnected.
///
...
fn has_internet_access(connectivity_level: NetworkConnectivityLevel) -> bool {
   connectivity_level == NetworkConnectivityLevel::InternetAccess
}
...
/// We treat fixed-cost and variable-cost plans as metered, and unrestricted /
/// unknown as not metered.
...
/// come from
/// [`ConnectionCost`](https://learn.microsoft.com/en-us/uwp/api/windows.networking.connectivity.connectioncost?view=winrt-28000).
fn is_constrained(connection_cost: &ConnectionCost) -> Result<bool> {
...
mod tests {
   ...
   #[test]
   fn detects_internet_access_only_for_internet_access_level() {
      assert!(has_internet_access(
         NetworkConnectivityLevel::InternetAccess
      ));
      assert!(!has_internet_access(NetworkConnectivityLevel::None));
      assert!(!has_internet_access(NetworkConnectivityLevel::LocalAccess));
      assert!(!has_internet_access(
