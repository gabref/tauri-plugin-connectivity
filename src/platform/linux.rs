...
// NetworkManager D-Bus enum values used by this backend.
...
fn network_manager_connection_status(connection: &Connection) -> zbus::Result<ConnectionStatus> {
   ...
   // `Connectivity` is a cached property. `FULL` is the only state that maps to
   // internet reachability for this API; `UNKNOWN` falls back to NM's broader
   // networking state.
   let connectivity = manager.get_property::<u32>("Connectivity")?;
   debug!(connectivity, "queried NetworkManager connectivity state");

   let connected = match map_connectivity(connectivity) {
   ...
   Ok(ConnectionStatus {
      ...
      constrained: is_constrained(details.metered, details.roaming),
...
fn fallback_connection_status() -> ConnectionStatus {
   ...
   // routing table through /proc. A non-loopback default route is the strongest
   ...
   let Some(iface) = default_route_interface(&route_table) else {
      debug!("Linux route table does not contain a non-loopback default route");
      return ConnectionStatus::disconnected();
   };
...
fn is_constrained(metered: bool, roaming: bool) -> bool {
   metered || roaming
}
...
mod tests {
   ...
   fn maps_connectivity_states() {
      ...
      assert_eq!(
         map_connectivity(NM_CONNECTIVITY_UNKNOWN),
         ConnectedState::Unknown
      );
   }
   ...
   fn treats_metering_or_roaming_as_constrained() {
      assert!(!is_constrained(false, false));
      assert!(is_constrained(true, false));
      assert!(is_constrained(false, true));
      assert!(is_constrained(true, true));
   }
